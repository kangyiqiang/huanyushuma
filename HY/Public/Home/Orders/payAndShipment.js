...
function doDealBigShipCalenderInfo(dataResult) {
	...
	if(typeof dataResult.installJzdDates !=='undefined' && dataResult.installJzdDates!=null){
		 ...
		 $("#install_jzd_dates").val(JSON.stringify(dataResult.installJzdDates));
	 }
	if(typeof dataResult.installDates !=='undefined' && dataResult.installDates!=null){
		 
		$("#install_bzd_dates").val(JSON.stringify(dataResult.installJzdDates));
	}
}
...
function doAsynGetSkuPayAndShipInfo(typeFlag) {
  ...
  var param = "paymentId=" + payId;
  param = param + "&shipParam.reset311="+reset311 + "&resetFlag="+resetFlag+ "&shipParam.onlinePayType=" + otype +"&typeFlag="+typeFlag;
  var easyBuyFlag = $("#easyBuyFlag").val();
  if(easyBuyFlag == "1"||easyBuyFlag=="2"){
    param += "&ebf=" + easyBuyFlag;
  }
...
//      alert("selfPickOptimize:"+selfPickOptimize);
      if(selfPickOptimize == 1 && $("#selfPickSiteName").length==1 && !$("#selfPickSiteName").hasClass("item-selected")){
    	  ...
    	  if(jsonO!=null){
    		  var flag = 0;
		    	if(jsonO.usedPickView!=null && jsonO.usedPickView!=""){
		    		...
		    		if(upv.pickId!=0){
			    		...
			    		usedLi = usedLi + "<a href='#none' clstag='pageclick|keycount|trade_201602181|57' onclick='openEditSelfPickConsigneeDialog(2)' class='ftx-05 ml10 selfpick-edit'>修改自提点</a></div></li>";
		    	...
		    	if(jsonO.nearestPickView!=null && jsonO.nearestPickView!=""){
		    		...
		    		if(npv.pickId!=0){
			    		...
			    		nearestLi = nearestLi + "<a href='#none' clstag='pageclick|keycount|trade_201602181|57' onclick='openEditSelfPickConsigneeDialog(3)' class='ftx-05 ml10 selfpick-edit'>修改自提点</a></div></li>";
...
//					  	    		$(this).children('span').remove();
	...
	} else if (consignee_provinceId == overSeasId || consignee_provinceId == taiWanId) {
...
function shipmentTips618(flag) {
    ...
    if($("#needForJZDcalendar").val()==1){
    	calendarCopywritingContent=0;
    	if($("#jd_shipment_item").hasClass("curr") && !$("#zxj_show_id").hasClass("hide")){
    		calendarCopywritingContent = $("#calendarCopywritingContent").val();
    	}
    	if($("#bigitem_shipment_item").hasClass("curr") && !$("#djdJzd_show_id").hasClass("hide")){
    		calendarCopywritingContent = $("#calendarCopywritingContent").val();
    	}
    ...
    //日历弹窗引到话术
    if(calendarCopywritingContent != 0 && calendarCopywritingContent != undefined){
    	$(".tips-618.tips-618-for-calendar").find(".tips-m").html(calendarCopywritingContent);
