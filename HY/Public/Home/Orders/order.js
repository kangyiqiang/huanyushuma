seajs.use([
    "jdf/1.0.0/unit/globalInit/2.0.0/globalInit",
      ...
      if (consigneeResult.success) {
        ...
        if(consigneeResult.resetCommon){
        	if(consigneeResult.selectNormalInvoiceContent=="不开发票"){
        		...
        		//普票 
                $("#part-inv").html("<span class='mr10'>普通发票（纸质）&nbsp; </span><span class='mr10'> "+consigneeResult.selectInvoiceTitle+"&nbsp; </span><span class='mr10'>"+consigneeResult.selectNormalInvoiceContent+"&nbsp; </span><a onclick='edit_Invoice()' class='ftx-05 invoice-edit' href='#none'>修改</a>");
        	}else if(consigneeResult.selectInvoiceType==2){
        		//增值税发票
        		$("#part-inv").html("<span class='mr10'>增值税发票&nbsp; </span><span class='mr10'> "+consigneeResult.selectInvoiceTitle+"&nbsp; </span><span class='mr10'>"+consigneeResult.selectNormalInvoiceContent+"&nbsp; </span><a onclick='edit_Invoice()' class='ftx-05 invoice-edit' href='#none'>修改</a>");
        	}else if(consigneeResult.selectInvoiceType==3){
        		//电子发票 
        		$("#part-inv").html("<span class='mr10'>普通发票（电子）&nbsp; </span><span class='mr10'> "+consigneeResult.selectInvoiceTitle+"&nbsp; </span><span class='mr10'>"+consigneeResult.selectNormalInvoiceContent+"&nbsp; </span><a onclick='edit_Invoice()' class='ftx-05 invoice-edit' href='#none'>修改</a>");
        	}
        }
        ...
        for (var prop in venderFreight) {
            ...
            $("#tooltip-box06 b.free-freight").each(function() {
                ...
                if (prop != 0) { // pop
                    ...
                    // 普通基础运费
                    var baseFreight = freightSop - xuzhongfreight - baseSxFreight - xuzhongSxfreight - jzdfreight - jsdfreight - djzdfreight - fuelFreightOversea;
...
//            var hasJdExtFreight = 0;
            if(hasJdNormalFreight && hasJdSxFreight){ // 生鲜和非生鲜都存在时，增加分隔线
                ...
                if(hasJdNormalFreight){
                    $("#sx-freight-container").hide();
                    if(jsdfreight>0 || jzdfreight>0 || jzdsxfreight>0 || jzdsxdjfreight>0 || jsdsxfreight>0 || djzdfreight>0){
                    	$("#ext-freight-split-line").show();
                    }
                    //港澳地区不显示标题、海外不显示标题
...
//港澳售项目 继续提交订单
//全球售
function continue_SubmitOrder(isOversea){
	if (isOversea == 1) {
		$("#overseamtbuy").attr("checked",true);
	} else {
	...
	submit_Order();
}window.continue_SubmitOrder=continue_SubmitOrder;
/**
 * 提交订单方法
 */
  	...
  	if(consignee_area!=null){
	  	...
	  	if(consignee_provinceId == hongKongId || consignee_provinceId == overSeasId || consignee_provinceId == taiWanId){
			...
			if($(".hknostock").length>0){// 港澳台、海外公用一个无货class
				...
				if ((consignee_provinceId == hongKongId && $("#hkmtbuy").is(":checked"))
						|| ((consignee_provinceId == overSeasId || consignee_provinceId == taiWanId) && $("#overseamtbuy").is(":checked"))) {
					canSubmitFlag=true;
				} else {
					//弹框 继续提交提示
					var a;
			...
			if ($(this).attr("id") == venderId) {
				...
				if (1 == venderInsurance.type) {
					...
					if (venderInsurance.venderName != null && venderInsurance.venderName != "undefined") {
						...
						$(this).html(showVenderName + "为您购买了运费险，最高赔付25.00元/单");
						// 展示
